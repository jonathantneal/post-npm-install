...
export default function postNpmInstall(useCi = false) {
  getPackageJsonFilename()
    ...
    .then(useCi => {
      execSync(useCi ? "npm ci" : "npm install", {
...
const canUseCi = optForCi =>
  Promise.all([checkNpmCiExist(), doesPackageLockJsonExist()]).then(
    ([hasCiCmd, hasLock]) => {
      if (!hasLock && optForCi) {
        console.warn(
...
// need to trim the \n that gets returned
const checkNpmCiExist = () =>
  exec("npm ci --help").then(output => output.toString().trim() === "npm ci");

const doesPackageLockJsonExist = () =>
  new Promise(resolve => {
    access("package-lock.json", fsconstants.R_OK, error => {
      resolve(!error);
    });
  });

// return whether the values of two objects differ
const haveObjectValuesChanged = (dep1 = {}, dep2 = {}) =>
  Object.keys(Object(dep1))
